...
for(let i = 0; i < movies.length; i++) {
  console.log(`Number of actors in the move ${movies[i].title} = ${movies[i].actors.length}`);
}

// 26. Log the number of genres in all three movies one by one
for(let i = 0; i < movies.length; i++) {
  console.log(`Number of genres in the move ${movies[i].title} = ${movies[i].genres.length}`);
}
